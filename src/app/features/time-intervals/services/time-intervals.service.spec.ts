...
describe('TimeIntervalsService', () => {
  ...
  describe('Time intervals management', () => {
    ...
    it('should compare the columns and return  an array with 31 days and 288 records in each day for interval = 5 ', () => {
      const observable = service.getComparedColumns(INTERVAL_RANGE_IN_MINUTES.EVERY_FIVE, headingIntervalsFullValues, daysRowsData, 31)
    ...
    it('should compare the columns and return  an array with 31 days and 48 records in each day for interval = 30 ', () => {
      const observable = service.getComparedColumns(INTERVAL_RANGE_IN_MINUTES.EVERY_THIRTY, headingIntervalsFullValues, daysRowsData, 31)
      observable.subscribe(data => {
        expect(data.length).toEqual(31)
    ...
    it('should compare the columns and return  an array with 31 days and 24 records in each day for interval = 60 ', () => {
      const observable = service.getComparedColumns(INTERVAL_RANGE_IN_MINUTES.EVERY_SIXTY, headingIntervalsFullValues, daysRowsData, 31)
      observable.subscribe(data => {
        expect(data.length).toEqual(31)
